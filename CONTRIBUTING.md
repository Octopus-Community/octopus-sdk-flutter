...
The example app reads its demo API key from `--dart-define`, injected at build time. **No key is ever committed to this repo** (the public mirror ships none — bring your own). Internally, keys live in the pm-tools `shared/config/secrets.local.yaml` source of truth; run the example with:
