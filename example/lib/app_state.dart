...
/// [start] routes the SDK to `api-demo2.8pus.io` via [ApiServer] when it is
...
class AppState extends ChangeNotifier {
  ...
  Future<void> start(DemoConfig config) async {
    ...
    try {
      ...
      // `ApiServer`. We use the canonical demo2 host pm-tools advertises
      // (`api-demo2.8pus.io`) — the same one the iOS / Android samples
      // hit in their own QA configs.
      final apiServer = octopusServer.trim().toLowerCase() == 'demo2'
          ? ApiServer(host: 'api-demo2.8pus.io')
          : null;
      // No `appManagedFields` here on purpose. The Flutter sample tests
      // against the pm-tools demo2 keys, each of which the backend issued
  ...
  /// (`api-demo2.8pus.io` when launched with `--dart-define=OCTOPUS_SERVER=demo2`)
